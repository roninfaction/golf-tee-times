...
// Width constants — must be consistent between header and data rows
const HOLE_W = 28;
const NAME_W = 80;
const SUB_W = 36;
const TOT_W = 42;
...
export default async function SharePage({ params }: Params) {
  ...
  // ── Best-ball per-hole computation ──────────────────────────────────────
  ...
  // ── Scramble team totals ────────────────────────────────────────────────
  const scrambleTotals: Record<string, number> = {};
  if (isScramble) {
    for (const team of teams) {
      const members = scoresByTeam[team.id] ?? [];
      if (members.length > 0) {
        scrambleTotals[team.id] = Math.min(...members.map(s => s.gross_score));
      }
    }
  }

  // ── Sort teams for results display ──────────────────────────────────────
  const sortedTeams = teams.filter(t => scoresByTeam[t.id]).sort((a, b) => {
    if (isBestBall) {
      const hwDiff = (teamHolesWon[b.id] ?? 0) - (teamHolesWon[a.id] ?? 0);
      if (hwDiff !== 0) return hwDiff;
      return (teamBestTotals[a.id] ?? 999) - (teamBestTotals[b.id] ?? 999);
    }
            ...
            <div style={{ background: "rgba(255,255,255,0.055)", border: "0.5px solid rgba(80,200,110,0.16)", borderRadius: 14, overflow: "hidden" }}>
              {sortedTeams.map((team, i) => {
                ...
                return (
                  <div
                    key={team.id}
                    style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderBottom: isLast ? "none" : "0.5px solid rgba(80,200,110,0.10)" }}
                  >
                    <div style={{ width: 10, height: 10, borderRadius: "50%", background: team.color ?? "#fff", flexShrink: 0 }} />
                    {isFirst
                      ? <Trophy size={14} style={{ color: GOLD, flexShrink: 0 }} />
                      : <div style={{ width: 14, flexShrink: 0 }} />}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "#fff" }}>{team.name}</p>
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      ...
                      {bbTotal != null && (
                        <p style={{ margin: 0, fontSize: 11, color: "rgba(255,255,255,0.3)" }}>{bbTotal} strokes</p>
                      )}
                    </div>
                  </div>
                );
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.055)", border: "0.5px solid rgba(80,200,110,0.16)", borderRadius: 14, overflow: "hidden" }}>
              {sortedTeams.map((team, i) => (
                <div
                  key={team.id}
                  style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: i < sortedTeams.length - 1 ? "0.5px solid rgba(80,200,110,0.10)" : "none", borderLeft: `3px solid ${team.color ?? "rgba(255,255,255,0.2)"}` }}
                >
                  {i === 0
              ...
              {allScores.map((s, i) => {
                ...
                return (
                  <div key={s.id} style={{ display: "flex", alignItems: "center", padding: "10px 16px", borderBottom: i < allScores.length - 1 ? "0.5px solid rgba(80,200,110,0.10)" : "none" }}>
                    ...
                    {isFirst && !isBestBall && !isScramble
                      ? <Trophy size={13} style={{ color: GOLD, marginRight: 6, flexShrink: 0 }} />
                      : <div style={{ width: 19 }} />}
                    <span style={{ flex: 1, fontWeight: 600 }}>{playerName(s)}</span>
                    {team && (
                      <span style={{ fontSize: 10, fontWeight: 700, color: team.color ?? "rgba(255,255,255,0.4)", background: "rgba(255,255,255,0.06)", borderRadius: 6, padding: "2px 6px", marginRight: 10 }}>
                        {team.name}
                      </span>
                    )}
          ...
          <div>
            ...
            <div style={{ background: "rgba(255,255,255,0.055)", border: "0.5px solid rgba(80,200,110,0.16)", borderRadius: 14, overflowX: "auto", WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
              <div style={{ minWidth: NAME_W + HOLE_W * 18 + SUB_W * 2 + TOT_W + 20 }}>
                ...
                {/* Render by team group (or ungrouped) */}
                {(() => {
