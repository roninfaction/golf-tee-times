...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const dateStr = dateObj && !isNaN(dateObj.getTime())
    ? dateObj.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric", timeZone: "America/Los_Angeles" })
    : null;
  ...
  if (tt.course_place_id) {
    const { data: courseRaw } = await svc.from("courses").select("photo_uri").eq("place_id", tt.course_place_id).maybeSingle();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "website",
      ...(photoUri ? { images: [{ url: photoUri, width: 1200, height: 630, alt: courseName }] } : {}),
    },
    twitter: {
